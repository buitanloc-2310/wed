...
interface AdminPagesManagerProps {
  ...
  onUpdateCustomPage: (id: string, updates: Partial<CustomPage>) => Promise<boolean>;
  onAddCustomPage: (page: CustomPage) => Promise<boolean>;
  onDeleteCustomPage: (id: string) => Promise<boolean>;
...
export const AdminPagesManager: React.FC<AdminPagesManagerProps> = ({
  ...
  const handleCreateNewPage = async () => {
    ...
    const created = await onAddCustomPage(newPage);
    if (!created) { onShowToast('Chưa thể tạo trang trên máy chủ.'); return; }
  ...
  const handleBatchPublish = async () => {
    const results = await Promise.all(selectedIds.map((id) => onUpdateCustomPage(id, { isPublished: true })));
    if (results.some(r => !r)) { onShowToast('Một số trang chưa thể xuất bản. Vui lòng thử lại.'); return; }
  ...
  const handleBatchDraft = async () => {
    const results = await Promise.all(selectedIds.map((id) => onUpdateCustomPage(id, { isPublished: false })));
    if (results.some(r => !r)) { onShowToast('Một số trang chưa thể chuyển về bản nháp.'); return; }
  ...
  const handleBatchDelete = async () => {
    if (confirm(`Bạn có chắc chắn muốn xóa ${selectedIds.length} trang đã chọn?`)) {
      const results = await Promise.all(selectedIds.map((id) => onDeleteCustomPage(id)));
      if (results.some(r => !r)) { onShowToast('Một số trang chưa thể xóa trên máy chủ.'); return; }
  ...
  const handleConfirmDelete = async () => {
    ...
    const deleted = await onDeleteCustomPage(pageToDelete.id);
    if (!deleted) { onShowToast('Chưa thể xóa trang trên máy chủ.'); return; }
        ...
        <div className="h-16 bg-white border-b border-slate-200 px-4 sm:px-6 flex items-center justify-between gap-4 sticky top-0 z-30 shadow-2xs">
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            <button
              type="button"
              onClick={async () => {
                const ok = await onUpdateCustomPage(activeCustomPage.id, { isPublished: true });
                if (!ok) { onShowToast('Chưa thể xuất bản lên máy chủ. Nội dung vẫn được giữ trong trình chỉnh sửa.'); return; }
            ...
            <div className="bg-white rounded-2xl max-w-3xl w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
              ...
              <div className="p-6 sm:p-8 overflow-y-auto space-y-4">
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{ __html: sanitizeHtml(rawContentHtml) }}
                />
              </div>
