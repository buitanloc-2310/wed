...
export async function onRequestGet(context) {
  ...
  const listed = await context.env.MEDIA.list({ prefix: 'website/', limit: 100, cursor, include: ['httpMetadata', 'customMetadata'] });
  return json({
    ok: true,
    items: listed.objects.map((o) => ({ key: o.key, url: `/media/${o.key}`, size: o.size, uploaded: o.uploaded, type: o.httpMetadata?.contentType || '', name: o.customMetadata?.originalName || o.key.split('/').pop() })),
