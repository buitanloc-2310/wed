...
export const ImageUrlInput: React.FC<ImageUrlInputProps> = ({
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      ...
      const data = await adminApi('/api/media/upload', { method:'POST', body:form });
      if (!data?.url) throw new Error('Máy chủ không trả về địa chỉ ảnh.');
      handleInputChange(data.url);
    } catch (error:any) { setUploadError(error?.message || 'Không thể tải ảnh lên.'); }
