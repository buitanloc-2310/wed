...
export const AdminLayoutManager: React.FC<Props> = ({ onShowToast, onSwitchTab }) => {
  ...
  const open = (key:SectionKey) => {
    const data:Record<SectionKey,Record<string,string>>={
      ...
      direction:{directionLabel:siteConfig.directionLabel||'',directionText:siteConfig.directionText||''},
  ...
  const save=()=>{ if(!editing)return; updateSiteConfig(draft as Partial<SiteConfig>); onShowToast?.(`Đã lưu nội dung “${LABELS[editing]}”.`); setEditing(null); };
  ...
  return <div className="space-y-6">
    ...
    <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
      Chân trang không còn nằm trong “Quản lý Trang chủ”. Nội dung chân trang dùng cấu trúc đã chốt của Sky First Network và được quản lý tại Cài đặt hệ thống.
    </div>
