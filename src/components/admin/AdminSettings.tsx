...
export const AdminSettings: React.FC<AdminSettingsProps> = ({
  ...
  const handleSaveBrand = async () => {
    ...
    onShowToast(ok ? 'Đã lưu nhận diện thương hiệu và đồng bộ lên website.' : 'Đã cập nhật trên trình duyệt nhưng chưa ghi được lên Firebase. Hãy kiểm tra quyền hoặc kết nối.');
  };
  ...
  const handleLogoFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > 500 * 1024) {
        onShowToast('Ảnh logo tải trực tiếp nên nhỏ hơn 500 KB để tránh vượt giới hạn tài liệu Firestore. Có thể dùng URL ảnh thay thế.');
        return;
      }
  ...
  const handleSaveSiteStatus = async () => {
    ...
    if (!ok) { onShowToast('Thay đổi chưa ghi được lên Firebase. Website công khai có thể chưa nhận cài đặt mới.'); return; }
