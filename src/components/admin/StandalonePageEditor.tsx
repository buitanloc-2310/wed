...
export const StandalonePageEditor: React.FC<StandalonePageEditorProps> = ({
  ...
  const [visionContent, setVisionContent] = useState(
    page.visionContent ||
      'Trở thành mạng lưới thanh niên và giáo dục cộng đồng hàng đầu tại Việt Nam, truyền cảm hứng và trang bị năng lực tự lập, tự cường, hội nhập toàn cầu cho hơn 100.000 bạn trẻ.'
  );
  ...
  const [missionContent, setMissionContent] = useState(
    page.missionContent ||
      'Khơi mở tiềm năng bản thân, trao quyền hành động cho thế hệ trẻ qua các mô hình giáo dục thực nghiệm (Service-Learning), giải quyết các thách thức xã hội thiết thực tại địa phương.'
  );
  ...
  const [teamSectionSubtitle, setTeamSectionSubtitle] = useState(
    page.teamSectionSubtitle || 'Ban Sáng lập & Thường trực SFN'
  );
  ...
  const [unitsSectionTitle, setUnitsSectionTitle] = useState(
    page.unitsSectionTitle || '5 Đơn Vị Chuyên Môn Của SFN'
  );
  ...
  const [ctaTitle, setCtaTitle] = useState(page.ctaTitle || 'Cùng SFN Đồng Hành Phụng Sự Xã Hội');
  ...
  const [certSearchPlaceholder, setCertSearchPlaceholder] = useState(
    page.certSearchPlaceholder || 'Nhập mã chứng nhận (Ví dụ: SFN-2026-OK)'
  );
  const [certGuidanceNote, setCertGuidanceNote] = useState(
    page.certGuidanceNote ||
      'Hệ thống tra cứu tự động đối chiếu mã số với cơ sở dữ liệu số hóa SFCA theo thời gian thực.'
  );
  ...
  const [certFeature1Desc, setCertFeature1Desc] = useState(
    page.certFeature1Desc ||
      'Mỗi chứng nhận được cấp một mã số duy nhất theo định dạng SFN-[NĂM]-[MÃ], không trùng lặp.'
  );
  ...
  const [certFeature2Desc, setCertFeature2Desc] = useState(
    page.certFeature2Desc ||
      'Hồ sơ được lưu trữ trên cơ sở dữ liệu số của SFN phục vụ đối chiếu năng lực và du học.'
  );
  ...
  const [certFeature3Desc, setCertFeature3Desc] = useState(
    page.certFeature3Desc ||
      'Cần xác minh bổ sung hoặc chỉnh sửa thông tin, vui lòng gửi yêu cầu qua Trang Liên Hệ SFN.'
  );
  ...
  const [certCtaDescription, setCertCtaDescription] = useState(
    page.certCtaDescription ||
      'Đội ngũ điều phối và kỹ thuật viên SFCA luôn sẵn sàng giải đáp và xử lý yêu cầu xác minh thông tin.'
  );
  ...
  const [sponsorCommit1Title, setSponsorCommit1Title] = useState(
    page.sponsorCommit1Title || '100% Sao Kê Công Khai'
  );
  ...
  const [sponsorCommit2Title, setSponsorCommit2Title] = useState(
    page.sponsorCommit2Title || 'Chứng Nhận Số SFCA'
  );
  const [sponsorCommit2Desc, setSponsorCommit2Desc] = useState(
    page.sponsorCommit2Desc ||
      'Cấp Giấy chứng nhận tri ân điện tử có mã định danh và QR code quét tra cứu trực tuyến toàn quốc.'
  );
  ...
  const [sponsorCommit4Desc, setSponsorCommit4Desc] = useState(
    page.sponsorCommit4Desc ||
      'Đồng hành cùng Trung tâm Truyền thông SFMC để tôn vinh những nghĩa cử nhân văn đến cộng đồng người trẻ.'
  );

  // 4.3 Packages
  const [sponsorPackagesHeading, setSponsorPackagesHeading] = useState(
    page.sponsorPackagesHeading || 'Các Hình Thức Đồng Hành Cùng SFN'
  );
  ...
  const [sponsorPkg1Unit, setSponsorPkg1Unit] = useState(
    page.sponsorPkg1Unit || 'Đồng hành cùng SFEC & SFIR'
  );
  ...
  const [sponsorPkg2Unit, setSponsorPkg2Unit] = useState(
    page.sponsorPkg2Unit || 'Đồng hành cùng SFYC (Tình nguyện)'
  );
  ...
  const [sponsorPkg3Unit, setSponsorPkg3Unit] = useState(
    page.sponsorPkg3Unit || 'Đồng hành cùng SFMC & SFCA'
  );
  ...
  const [sponsorPkg4Title, setSponsorPkg4Title] = useState(
    page.sponsorPkg4Title || 'Đối Tác Chiến Lược Dài Hạn (MOU)'
  );
  ...
  const [sponsorPkg4Unit, setSponsorPkg4Unit] = useState(
    page.sponsorPkg4Unit || 'Đồng hành cùng Ban Điều hành Sky First Network'
  );
  const [sponsorPkg4Desc, setSponsorPkg4Desc] = useState(
    page.sponsorPkg4Desc ||
      'Ký kết Biên bản Ghi nhớ (MOU) hợp tác chiến lược theo năm. Đồng tổ chức diễn đàn thanh niên, bảo trợ truyền thông và đặt hàng đề tài nghiên cứu xã hội SFIR.'
  );
  ...
  const [sponsorBankAccount, setSponsorBankAccount] = useState(
    page.sponsorBankAccount || '0337775329'
  );
  ...
  const [sponsorQrCodeUrl, setSponsorQrCodeUrl] = useState(
    page.sponsorQrCodeUrl || 'https://img.vietqr.io/image/MB-0337775329-compact2.png?amount=0&addInfo=TAITRO%20SFN&accountName=SKY%20FIRST%20NETWORK'
  );
  ...
  const [sponsorEmail, setSponsorEmail] = useState(page.sponsorEmail || 'sponsor@skyfirst.io.vn');
  const [sponsorContactLeadTitle, setSponsorContactLeadTitle] = useState(
    page.sponsorContactLeadTitle || 'Ban Đối Ngoại & Hợp Tác SFN'
  );
  const [sponsorContactHeading, setSponsorContactHeading] = useState(
    page.sponsorContactHeading || 'Đồng Hành & Hợp Tác Cùng SFN'
  );
  ...
  const [sponsorContactEmail, setSponsorContactEmail] = useState(
    page.sponsorContactEmail || 'sponsor@skyfirst.network'
  );

  // 4.5 FAQs
  const [sponsorFaqHeading, setSponsorFaqHeading] = useState(
    page.sponsorFaqHeading || 'Câu Hỏi Thường Gặp Về Tài Trợ SFN'
  );
  ...
  const [sponsorFaq1Q, setSponsorFaq1Q] = useState(
    page.sponsorFaq1Q || 'Cá nhân có thể đóng góp hoặc tài trợ cho SFN không?'
  );
  const [sponsorFaq1A, setSponsorFaq1A] = useState(
    page.sponsorFaq1A ||
      'Hoàn toàn được. SFN trân trọng mọi sự đồng hành từ cá nhân, sinh viên, cựu tình nguyện viên đến các chuyên gia. Bạn có thể ủng hộ theo từng dự án cụ thể hoặc gửi vào Quỹ Phát triển Thanh niên SFN với bất kỳ số tiền nào.'
  );
  ...
  const [sponsorFaq2A, setSponsorFaq2A] = useState(
    page.sponsorFaq2A ||
      'SFN cam kết 100% minh bạch tài chính. Tất cả khoản đóng góp đều được cập nhật vào Bảng thu chi công khai theo thời gian thực. Sau mỗi dự án, Ban Tài chính gửi Báo cáo quyết toán kèm toàn bộ chứng từ hóa đơn và nghiệm thu hình ảnh trực tiếp qua email của nhà tài trợ.'
  );
  ...
  const [sponsorFaq3A, setSponsorFaq3A] = useState(
    page.sponsorFaq3A ||
      'Có. Toàn bộ nhà tài trợ và đối tác đồng hành đều được Ban Quản lý Chứng nhận Số (SFCA) cấp Giấy Chứng Nhận Tri Ân Điện Tử có mã số định danh riêng và mã QR xác thực trực tuyến tại Cổng Tra cứu Giấy chứng nhận của SFN.'
  );
  ...
  const [sponsorFaq4A, setSponsorFaq4A] = useState(
    page.sponsorFaq4A ||
      'SFN hợp tác với các đơn vị tổ chức xã hội và quỹ hợp pháp được công nhận. Các chương trình tài trợ giáo dục và từ thiện đủ điều kiện theo quy định của pháp luật hiện hành đều có thể xuất biên bản tiếp nhận phục vụ hạch toán chi phí hợp lý.'
  );

  // 4.6 Quick CTA
  const [sponsorCtaTitle, setSponsorCtaTitle] = useState(
    page.sponsorCtaTitle || 'Bạn muốn trao đổi trực tiếp cùng Ban Điều hành Sky First Network?'
  );
  const [sponsorCtaDescription, setSponsorCtaDescription] = useState(
    page.sponsorCtaDescription ||
      'Vui lòng gọi hotline đối ngoại hoặc đặt lịch hẹn gặp trực tiếp tại văn phòng điều phối SFN.'
  );
  ...
  const [joinRole1Title, setJoinRole1Title] = useState(
    page.joinRole1Title || 'Tình Nguyện Viên Chiến Dịch (SFYC)'
  );
  ...
  const [joinRole1Highlight1, setJoinRole1Highlight1] = useState(
    page.joinRole1Highlight1 || 'Nhận Giấy chứng nhận số SFCA có mã QR định danh toàn quốc'
  );
  ...
  const [joinRole2Title, setJoinRole2Title] = useState(
    page.joinRole2Title || 'Thành Viên Ban Điều Hành & Core Team'
  );
  const [joinRole2Tag, setJoinRole2Tag] = useState(page.joinRole2Tag || 'CORE TEAM SFN');
  const [joinRole2Description, setJoinRole2Description] = useState(
    page.joinRole2Description ||
      'Trực tiếp tham gia quản trị, xây dựng nội dung giáo dục (SFEC), truyền thông thương hiệu (SFMC), hoặc nghiên cứu chuyển đổi số (SFIR).'
  );
  ...
  const [joinRole2ButtonLabel, setJoinRole2ButtonLabel] = useState(
    page.joinRole2ButtonLabel || 'Ứng Tuyển Ban Điều Hành'
  );
  ...
  const [joinRole2Highlight2, setJoinRole2Highlight2] = useState(
    page.joinRole2Highlight2 || 'Chứng nhận bổ nhiệm và thư giới thiệu từ Ban Lãnh đạo SFN'
  );
  ...
  const [joinRole2Highlight4, setJoinRole2Highlight4] = useState(
    page.joinRole2Highlight4 || 'Cơ hội đại diện SFN tham dự các diễn đàn và hội nghị quốc gia'
  );
  ...
  const [joinRole3Period, setJoinRole3Period] = useState(
    page.joinRole3Period || 'Hợp tác thường niên / Theo MOU'
  );
  ...
  const [joinRole3Highlight2, setJoinRole3Highlight2] = useState(
    page.joinRole3Highlight2 || 'Bảo trợ kỹ thuật xác thực chứng nhận số miễn phí qua hệ thống SFCA'
  );
  ...
  const [joinCtaDescription, setJoinCtaDescription] = useState(
    page.joinCtaDescription ||
      'Hệ thống tiếp nhận hồ sơ tập trung của Ban Nhân Sự & Đối Ngoại SFN sẵn sàng hỗ trợ phản hồi trong vòng 24 giờ làm việc.'
  );
  const [joinCtaButtonLabel, setJoinCtaButtonLabel] = useState(
    page.joinCtaButtonLabel || 'Đến Trang Liên Hệ SFN'
  );
  ...
  const [joinAddress, setJoinAddress] = useState(
    page.joinAddress || 'Hà Nội & TP. Hồ Chí Minh, Việt Nam'
  );
  ...
  const [joinFaqHeading, setJoinFaqHeading] = useState(
    page.joinFaqHeading || 'Câu Hỏi Thường Gặp Về Gia Nhập SFN'
  );
  ...
  const [joinFaq1Q, setJoinFaq1Q] = useState(
    page.joinFaq1Q || 'Ai có thể đăng ký làm Tình nguyện viên của SFN?'
  );
  const [joinFaq1A, setJoinFaq1A] = useState(
    page.joinFaq1A || 'Mọi bạn trẻ (học sinh THPT, sinh viên, cựu sinh viên và người đi làm) có tinh thần trách nhiệm, yêu thích hoạt động cộng đồng và mong muốn cống hiến đều có thể đăng ký tham gia các chiến dịch SFYC.'
  );
  const [joinFaq2Q, setJoinFaq2Q] = useState(
    page.joinFaq2Q || 'Ứng tuyển vào Ban Điều Hành (Core Team) có yêu cầu kinh nghiệm trước không?'
  );
  const [joinFaq2A, setJoinFaq2A] = useState(
    page.joinFaq2A || 'SFN đánh giá cao thái độ học hỏi, sự chủ động và tinh thần cam kết. Dù bạn chưa có nhiều kinh nghiệm, SFN luôn có lộ trình đào tạo nội bộ và cố vấn (mentorship) từ các anh chị đi trước.'
  );
  const [joinFaq3Q, setJoinFaq3Q] = useState(
    page.joinFaq3Q || 'Giấy chứng nhận số SFCA sau mỗi chiến dịch có giá trị như thế nào?'
  );
  const [joinFaq3A, setJoinFaq3A] = useState(
    page.joinFaq3A || 'Mỗi GCN cấp qua SFCA đều có mã định danh và QR code tra cứu trực tuyến tại /certificate, ghi nhận chính xác vai trò và số giờ tình nguyện phục vụ việc xét học bổng, du học hoặc CV xin việc.'
  );
  ...
  const [joinFaq4A, setJoinFaq4A] = useState(
    page.joinFaq4A || 'Sau khi bạn gửi thông tin qua form hoặc email, Ban Nhân sự SFN sẽ gửi thư xác nhận tự động và liên hệ lại trong vòng 24 - 48 giờ làm việc để xếp lịch phỏng vấn/trao đổi trực tiếp.'
  );
  ...
  const [contactFormTitle, setContactFormTitle] = useState(
    page.contactFormTitle || 'Gửi Tin Nhắn Đến Ban Điều Hành'
  );
  ...
  const [contactSubmitButtonLabel, setContactSubmitButtonLabel] = useState(
    page.contactSubmitButtonLabel || 'Gửi Tin Nhắn Đến SFN'
  );
  ...
  const [contactFaq1Q, setContactFaq1Q] = useState(
    page.contactFaq1Q || 'Thời gian Ban Điều hành Sky First Network tiếp nhận và phản hồi email là bao lâu?'
  );
  ...
  const [contactFaq2Q, setContactFaq2Q] = useState(
    page.contactFaq2Q || 'Tôi muốn đặt lịch làm việc trực tiếp tại văn phòng SFN thì cần làm gì?'
  );
  ...
  const [contactFaq3A, setContactFaq3A] = useState(
    page.contactFaq3A ||
      'Bạn có thể chọn trực tiếp đơn vị mong muốn (SFEC, SFYC, SFIR, SFMC, SFCA) tại mục "Đơn vị muốn kết nối" trong biểu mẫu liên hệ, thông tin sẽ được tự động gửi tới email nội bộ của trưởng đơn vị đó.'
  );
