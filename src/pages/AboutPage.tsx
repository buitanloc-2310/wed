...
export const AboutPage: React.FC<AboutPageProps> = ({ onNavigate }) => {
  ...
  const badgeText = pageData?.badge || 'HỆ SINH THÁI GIÁO DỤC SFN 2026';
  const titleText = pageData?.title || 'Về Sky First Network';
  const subtitleText = pageData?.summary || 'Mạng lưới Giáo dục & Phát triển Cộng đồng — Nơi người trẻ kết nối, kiến tạo tri thức, rèn luyện kỹ năng thực chiến và đồng hành phụng sự xã hội bền vững.';

  const visionBadge = pageData?.visionBadge || 'Định Hướng Chiến Lược';
  const visionTitle = pageData?.visionTitle || 'Tầm Nhìn 2030';
  const visionContent = pageData?.visionContent || 'Trở thành mạng lưới thanh niên và giáo dục cộng đồng hàng đầu tại Việt Nam, truyền cảm hứng và trang bị năng lực tự lập, tự cường, hội nhập toàn cầu cho hơn 100.000 bạn trẻ.';

  const missionBadge = pageData?.missionBadge || 'Mục Tiêu Hành Động';
  const missionTitle = pageData?.missionTitle || 'Sứ Mệnh Phụng Sự';
  const missionContent = pageData?.missionContent || 'Khơi mở tiềm năng bản thân, trao quyền hành động cho thế hệ trẻ qua các mô hình giáo dục thực nghiệm (Service-Learning), giải quyết các thách thức xã hội thiết thực tại địa phương.';
  ...
  const teamSectionSubtitle = pageData?.teamSectionSubtitle || 'Ban Sáng lập & Thường trực SFN';
  ...
  const unitsSectionTitle = pageData?.unitsSectionTitle || '5 Đơn Vị Chuyên Môn Của SFN';
  ...
  // 8. CTA Banner
  const ctaTitle = pageData?.ctaTitle || 'Cùng SFN Đồng Hành Phụng Sự Xã Hội';
  const ctaDescription = pageData?.ctaDescription || 'Dù bạn là học viên, tình nguyện viên hay đối tác tổ chức, cánh cửa của Sky First Network luôn rộng mở đón chào bạn.';
