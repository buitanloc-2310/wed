...
const MENU_ITEMS: MenuItemConfig[] = [
  { id: 'visual-editor', label: 'Chỉnh sửa Website', icon: <Globe size={18} />, largeIcon: <Globe size={24} /> },
  { id: 'system-health', label: 'Tình trạng hệ thống', icon: <AlertCircle size={18} />, largeIcon: <AlertCircle size={24} /> },
  {
    id: 'layout',
...
export const AdminPage: React.FC<AdminPageProps> = ({
  ...
  if (checkingSession) return <div className="min-h-screen bg-slate-50 flex items-center justify-center text-sm text-slate-500">Đang xác minh phiên quản trị...</div>;
    ...
    return (
      <StandalonePageEditor
        ...
        onSave={async (updates) => {
          const saved = await updateCustomPage(editingPage.id, updates);
        ...
        onShowToast={onShowToast}
        existingSlugs={customPages.filter((p) => p.id !== editingPage.id).map((p) => p.slug).filter(Boolean)}
      />
    );
  }
    ...
    <div className="flex min-h-screen bg-[#F8FAFC] font-sans text-slate-900">
      ...
      >
        ...
        <nav className="flex-1 space-y-1.5 overflow-y-auto p-3">
          {MENU_ITEMS.filter((item) => canOpenAdminTab(currentAdminUser.role, item.id)).map((item) => {
      ...
      <main className="flex min-w-0 flex-1 flex-col overflow-y-auto">
        ...
        <div className="mx-auto w-full max-w-6xl p-6">
          ...
          {activeTab === 'visual-editor' && canOpenAdminTab(currentAdminUser.role, 'visual-editor') && (
            <AdminVisualWebsiteEditor onShowToast={onShowToast} onNavigate={onNavigate} />
          )}

          {activeTab === 'system-health' && canOpenAdminTab(currentAdminUser.role, 'system-health') && (
            <AdminSystemHealth />
          )}
