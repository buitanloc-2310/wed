    ...
    <div className="bg-slate-50/50 min-h-screen pb-20">
      ...
      {article.isPublished === false && (
        <div className="bg-amber-500 text-white px-4 py-2.5 text-xs font-bold text-center flex items-center justify-center gap-2 shadow-sm">
          ...
          <span>Đây là bản nháp nội bộ — Bài viết chưa xuất bản công khai trên website Sky First Network.</span>
        </div>
      )}
      ...
      <article className="max-w-4xl mx-auto px-4 sm:px-6 pt-8">
        <div className="bg-white rounded-3xl p-6 sm:p-10 border border-slate-200/80 shadow-xs space-y-6">
          ...
          <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex items-center justify-between text-xs text-slate-600">
            <div>
              <span className="font-bold text-slate-900 block">Ban Truyền Thông Sky First Network</span>
              <span>Thông tin chính thức & bản quyền nội dung thuộc về Sky First Network.</span>
            </div>
            <button
              type="button"
              onClick={handleCopyUrl}
              className="px-3 py-1.5 bg-white border border-slate-200 text-[#0284C7] font-bold rounded-lg hover:bg-sky-50 transition shrink-0 ml-4"
            >
              Chia sẻ bài viết
            </button>
          </div>
